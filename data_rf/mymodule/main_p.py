...
from jadong import jadong_check
...
class MyApp(QDialog):
    ...
    def initUI(self):
        ...
        # 풀버젼
        # pyinstaller --hidden-import PyQt5 --hidden-import pyserial --hidden-import requests --hidden-import chardet --add-data="C:\\my_games\\rf_o_n\\data_rf;./data_rf" --add-data="C:\\my_games\\rf_o_n\\mysettings;./mysettings" --name rf_o_n -i="rf_start.ico" --add-data="rf_start.ico;./" --icon="rf_start.ico" --paths "C:\Users\1_S_3\AppData\Local\Programs\Python\Python311\Lib\site-packages\cv2" main.py
        # 업데이트버젼
...
class FirstTab(QWidget):
    ...
    def initUI(self):
        ...
        self.collection_on_off = QGroupBox('레어수집분해 On/Off')
        print("dark_demention", v_.onCollection)
        if v_.onCollection == True:
            tgl_now = "On"
        else:
            tgl_now = "Off"
        self.now_toggle = QLabel("레어 : " + tgl_now + "\n")
        # 토글 버튼
        ...
        cb6 = QComboBox()
        list6 = ['스케쥴 선택', '각종템받기', '거래소등록', '튜토육성', '퀘스트_서브', '퀘스트_국가', '일일미션_4', '일일미션_5', '일일미션_6', '일일미션_7', '일일미션_8', '일일미션_9']
        cb6.addItems(list6)
        vbox6 = QHBoxLayout()
        vbox6.addWidget(cb6)
        ...
        # 던전 종류
        self.dun_group_2 = QGroupBox('월드보스')
        ...
        # 던전 종류
        self.dun_group_3 = QGroupBox('지역던전')
        ...
        dun_g3_stair_list = ['층', '1', '2', '3', '4', '5', '6', '7', '8']
    ...
    def onActivated_slelect_collection_toggle(self, e):
        ...
        if v_.onCollection == True:
            ...
            tgl_now = "Off"
        self.now_toggle.setText("레어 : " + str(tgl_now) + "\n")
        self.tgl.setChecked(v_.onCollection)
        #self.set_rand_int()
    ...
    def onActivated_dunjeon_2_add(self):
        char_ = onCharacter
        dun_ = "월드_" + str(onDunjeon_2_level)
        if onCharacter == 0:
            pyautogui.alert(button='넵', text='캐릭터를 선택해 주시지예', title='뭐합니꺼')
        elif onCla == 'none':
            pyautogui.alert(button='넵', text='몇 클라인지 선택해 주시지예', title='뭐합니꺼')
        elif onDunjeon_2_level == 0 or onDunjeon_2_level == "층":
            pyautogui.alert(button='넵', text='던전 및 층수를 선택해 주시지예', title='아 진짜 뭐합니꺼')
        elif onCharacter != 0:
            print('char_', char_)
            print('dun_', dun_)
    ...
    def onActivated_dunjeon_3_add(self):
        char_ = onCharacter
        dun_ = "던전_" + str(onDunjeon_3) + "_" + str(onDunjeon_3_level)
        if onCharacter == 0:
            pyautogui.alert(button='넵', text='캐릭터를 선택해 주시지예', title='뭐합니꺼')
        elif onCla == 'none':
            pyautogui.alert(button='넵', text='몇 클라인지 선택해 주시지예', title='뭐합니꺼')
    ...
    def onActivated_hunt_add(self):
        ...
        elif onCharacter != 0 and onHunt != '< 벨라토 >':
            print('char_', char_)
            print('dun_', hun_)
    ...
    def onActivated_hunt_add_2(self):
        ...
        if onCharacter == 0:
            ...
            pyautogui.alert(button='넵', text='던전을 선택해 주시지예', title='뭐합니꺼')
        elif onCharacter != 0 and onHunt2 != '< 아크레시아 >':
            print('char_', char_)
            print('dun_', hun_)
    ...
    def onActivated_hunt_add_3(self):
        ...
        if onCharacter == 0:
            ...
            pyautogui.alert(button='넵', text='던전을 선택해 주시지예', title='뭐합니꺼')
        elif onCharacter != 0 and onHunt3 != '< 코라 >':
            print('char_', char_)
            print('dun_', hun_)
