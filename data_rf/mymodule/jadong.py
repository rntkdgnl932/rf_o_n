...
def jadong_mode_ex(cla, title_map, des_map):
    import numpy as np
    import cv2
            ...
            if imgs_ is not None and imgs_ != False:
                ...
                if imgs_ is not None and imgs_ != False:
                    ...
                    for i in range(3):
                        ...
                        full_path = "c:\\my_games\\rf_o_n\\data_rf\\imgs\\game_check\\out_check\\talk" + str(
                            is_list) + ".PNG"
                        img_array = np.fromfile(full_path, np.uint8)
                        img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                    ...
                    if is_data == True:
                        ...
                        img_array = np.fromfile(full_path, np.uint8)
                        img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                        imgs_ = imgs_set_(230, 970, 285, 1035, cla, img, 0.85)
                        if imgs_ is not None and imgs_ != False:
                            print("juljun_zero", imgs_)
                            potion_need = True
                        else:
